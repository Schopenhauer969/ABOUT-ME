...
export default function FolderFloat({
  ...
  sublabel = "",
  trigger = "none",
  defaultOpen = false, // Start closed so it animates when entering
  closeOnSelect = true,
  physics = true,
  drift = 0.5,
  onSelect,
  ...
  // Scroll observer that triggers open when entering viewport, and closes when leaving (allowing repeat)
  useEffect(() => {
    const observer = new IntersectionObserver(
      ([entry]) => {
        setIsInView(entry.isIntersecting);
        // Play animation when entering view, reset/close when completely out so it repeats on re-entry
